...
export function Timeline({ items, className, animate = true, orientation = "vertical" }: TimelineProps) {
  const statusColors = {
      ...
      <div className="space-y-6">
        {items.map((item, i) => {
          ...
          const wrapperProps = animate
            ? { initial: { opacity: 0, x: -20 }, animate: { opacity: 1, x: 0 }, transition: { delay: i * 0.1, duration: 0.3 } }
            : {}
              ...
              <div className="relative flex gap-4 pl-10">
                ...
                <div className="flex-1 pb-2">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {item.date && <time className="text-xs text-muted-foreground whitespace-nowrap">{item.date}</time>}
                  </div>
