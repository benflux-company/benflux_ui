  ...
  "relative w-full rounded-xl border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground",
  {
    variants: {
      variant: {
        ...
        success: "bg-green-500/10 border-green-500/30 text-green-700 dark:text-green-300 [&>svg]:text-green-500",
        warning: "bg-yellow-500/10 border-yellow-500/30 text-yellow-700 dark:text-yellow-300 [&>svg]:text-yellow-500",
...
function Alert({ className, variant = "default", dismissible, onDismiss, icon, children, ...props }: AlertProps) {
  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          <div role="alert" className={cn(alertVariants({ variant }), "relative", className)} {...props}>
